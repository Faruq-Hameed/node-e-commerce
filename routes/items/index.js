    ...
    const getItemsByQuery = async () => {
        try {
            ...
            const value = (req.query.maker)
                ? { maker: req.query.maker } : (req.query.itemName)
                    ? { itemName: req.query.itemName } : (req.query.category)
...
router.put('/:userId/:itemId', (req, res) => {
    ...
    const updateItem = async () => {
        ...
        await item.save()
        return res.status(200).send({ message: 'updated item successfully', item: item })
    }
