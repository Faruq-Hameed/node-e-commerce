...
router.get("/:userId", (req, res) => {
  ...
  if (req.params.userId === "u1")
    res.status(200).json({ "All Users Orders": allUsersOrders }); // for admin user & get all orders
  ...
  if (userOrder.userOrders.length === 0)
    return res.status(200).send("your cart is empty");

  let orderSummary = "below is your cart summary: \n \n"; //if user cart is not empty
  let i = 0;
  userOrder.userOrders.map((order) => {
    // trying to return user carts as a summarized sentences
    orderSummary += `\n ${++i}). orderId: ${order.orderId} ; productId ${
      order.productId
    } ; quantity: ${order.orderQty}kg;with value: $${order.orderValue} `;
  });
...
router.get("/:userId/:orderId", (req, res) => {
  const getOrderInfo = allUsersOrders.find((user) =>
    user.userOrders //getting the order by the order id
      .find((order) => order.orderId === req.params.orderId)
  );
  if (!getOrderInfo)
    return res.status(404).send("unknown request. The order doesn't exist");
  ...
  if (req.params.userId !== "u1" && getOrderInfo.userId !== req.params.userId)
    return res.status(401).send("unauthorized request/user");
  // an admin user can access any order with the id
  const orderToGet = getItem(
    getOrderInfo.userOrders,
    "orderId",
    req.params.orderId
  );
  ...
  const addItemToCart = async () => {
    try {
      ...
      const newItemToAddToCart = {
        //initiating the new item object to be added to the cart
        itemId: item._id,
        name: item.name,
        quantity: validation.value.quantity,
        price: item.price,
      };
      const valueOfTheNewItemToAddToCart =
        newItemToAddToCart.quantity * newItemToAddToCart.price;
      ...
      res.status(200).json({
        ...
        itemSummary: newItemToAddToCart,
        value: valueOfTheNewItemToAddToCart,
      });
    } catch (err) {
...
router.put("/:userId/:orderId", (req, res) => {
  ...
  const orderToUpdate = currentUserOrder.userOrders.find(
    (order) => order.orderId === req.params.orderId
  );
  if (!orderToUpdate) return res.status(404).send("no valid order found");
  if (!req.body.quantity || parseInt(req.body.quantity) * 0 !== 0)
    return res.status(404).send("provide a valid quantity");
  ...
  if (orderToUpdate.orderQty > parseInt(req.body.quantity)) {
    product.productQty =
      product.productQty +
      (orderToUpdate.orderQty - parseInt(req.body.quantity));
    product.soldQty =
      product.soldQty - (orderToUpdate.orderQty - parseInt(req.body.quantity));
    ...
    orderToUpdate.orderQty < parseInt(req.body.quantity) &&
    orderToUpdate.orderQty + product.productQty >= parseInt(req.body.quantity)
  ) {
    product.productQty =
      product.productQty -
      (parseInt(req.body.quantity) - orderToUpdate.orderQty);
    product.soldQty =
      product.soldQty + (parseInt(req.body.quantity) - orderToUpdate.orderQty);
    ...
    res
      .status(404)
      .send(
        `your new quantity is not available kindly buy ${
          parseInt(product.productQty) + orderToUpdate.orderQty
        }kg or less`
      );
});
...
router.delete("/:userId/:orderId", (req, res) => {
  const getOrderInfo = allUsersOrders.find((user) =>
    user.userOrders.find((order) => order.orderId === req.params.orderId)
  );
  if (!getOrderInfo)
    return res.status(404).send("unknown request. The order doesn't exist");
  ...
  const orderToDelete = getItem(
    getOrderInfo.userOrders,
    "orderId",
    req.params.orderId
  );
  const orderIndex = getOrderInfo.userOrders.findIndex(
    (order) => order.orderId === req.params.orderId
  );
